...
const searchPlayerEngagementsByPlayerId = async (req, res): Promise<void> => {
  const playerEngagements = await PlayerEngagementService.searchPlayerEngagementsByPlayerId(req.params.playerId);
  res.json(playerEngagements);
};
...
const updatePlayerEngagement = async (req, res): Promise<void> => {
  ...
  try {
    ...
    console.log("updated Engagement:", playerEngagementDataIn)
    const result = await PlayerEngagementService.updatePlayerEngagement(playerEngagementDataIn);
    ...
    if (result) {
      ...
      res.status(404).json({message: "Error: Failed to update player engagement"});
    }
  } catch (error) {
...
const addPlayerEngagement = async (req, res): Promise<void> => {
  try {
    const newPlayerEngagement = await PlayerEngagementService.addPlayerEngagement(req.body);
...
const addPlayerEngagementsBulk = async (req, res): Promise<void> => {
  try {
    const newPlayerEngagements = await PlayerEngagementService.addPlayerEngagementsBulk(req.body);
...
const deletePlayerEngagement = async (req, res): Promise<void> => {
  ...
  try {
    ...
    console.log(`true or false: ${deleted}`)
    if (deleted) {
      res.status(204).send();
    } else {
